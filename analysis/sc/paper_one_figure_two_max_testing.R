...
for (ela in elasticities_values){
  ...
  p1 <- ggplot()+
    ...
    scale_fill_npg() +geom_hline(yintercept = 0)+labs(x="",y="Max Bill Change", pattern="Mitigation decision",fill="Infrastructure")+
  ...
  title <- ggdraw() + draw_label(paste0("Max bill change, Elasticity: ",ela), fontface="bold")
  ...
  print(ela)
  ggsave(p1_DMDU,filename = paste0("~/Documents/__college/reseach_stuff/hr2w_droughts/analysis/sc/figure_2_plots/draft_figure_2_",ela,".png"),width=8,height=4,unit="in",scale=1.25)
