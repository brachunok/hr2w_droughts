...
# TODO: add a water use horizontal plot
...
bills_plot <- bills_plot +  geom_line(data = bills_m, aes(x = Date, y = dp),color="blue") +theme_bw()+xlim(c(ll_date,ul_date))
...
# add baseline
demand_plot <- demand_plot + geom_line(data=demand_c,aes(x=Date,y=dp),alpha=0.5,color="black",linetype="longdash")
demand_plot <- demand_plot + geom_line(data=demand_m,aes(x=Date,y=dp),alpha=0.5,color="blue",linetype="longdash") + theme_bw()


demand_plot
demand_plot_no_leg <-
  demand_plot + theme(legend.position = "none") + ylab("Monthly Water Billl ($)")+
  theme(axis.title.x=element_blank())
