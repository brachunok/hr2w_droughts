...
outputs <- outputs[which(outputs$price_elasticity==0),]
...
outputs_p1<- melt(outputs_perc_total,measure.vars = c("deep_poverty_max_avg","poverty_max_avg","near_poverty_max_avg","middle_class_max_avg","upper_class_max_avg","total_utility_cost"),
                  variable.name = "income_group",value.name = "annual_bill_change")


# now just look at the deep poverty and upper class groups 
outputs_p1 <- outputs_p1[which(outputs_p1$income_group%in%c("deep_poverty_max_avg","upper_class_max_avg","total_utility_cost")),]
...
p1 <- ggplot()+
  geom_bar_pattern(data=outputs_p1,aes(x=build_decision,y=annual_bill_change,fill=build_decision,pattern=mitigation_decision),stat='identity',position='dodge',color="black") +
  ...
  scale_fill_npg() +geom_hline(yintercept = 0)+labs(x="",y="Annual Bill Change (USD)", pattern="Mitigation decision",fill="Infrastructure")+
  scale_x_discrete(limits=build_order)
p1
...
# plot for Sarah's DMDU prez
p1_DMDU <- plot_grid(p1_leg,p1_no_leg,p1_tc_no_leg,nrow=1,rel_widths = c(.25,2,1))
p1_DMDU

#ggsave(p1_DMDU,filename = "~/Documents/__college/reseach_stuff/hr2w_droughts/analysis/sc/draft_figure_2_v1_DMDU_max.pdf",width=6,height=4,unit="in",scale=1.35)
