...
outputs_p1 <-outputs_p1[which(outputs_p1$drought_characteristic!="long_intense.csv"),]


# look at how reservoir size changes
build_order <- c("none","low_capacity","high_capacity")
...
p1 <- ggplot()+
  ...
  facet_grid(rows=vars(drought_characteristic),cols=vars(income_group)) + theme_bw() + scale_pattern_manual(values=c(market="none",baseline='circle'))+
