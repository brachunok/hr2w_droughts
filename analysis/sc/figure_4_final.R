...
#for (ela in elasticities_values){
  ...
  outputs_p1$label <- ""
  outputs_p1$label[which(outputs_p1$annual_bill_change%in%outputs_p1_agg$x)] <- "" # make this an X to add the labels back
  ...
  p1 <- ggplot()+
    ...
    facet_grid(rows=vars(drought_characteristic),cols=vars(income_group),scales="free_x") + theme_classic()  + scale_pattern_manual(values=c(market="none",baseline='circle'))+
  ...
  # 
  # p2 <- ggplot() + geom_line(data=bills_p2_melt,aes(x=date,y=value,color=variable))+theme_classic()()+scale_color_npg()+xlim(ll_date,ul_date)+
  ...
  # 
  # p3<- ggplot()+geom_bar(data= bills_bar_melt_1,aes(fill=variable,y=value,x=class),position="stack",stat="identity",alpha=0.45,width=width)+theme_classic()()+ylab("Yearly Change in \n Water Bills (USD) ")+
