...
for y in range(41):
    ...
    PED= 0.05*y  # the mean values of their analysis
    ...
    #for m in range(3):
        ...
        if outputs['trigger'].eq("YES").any() == False:
            ...
            decision_trigger = False
    # record the outputs
    outputs.to_csv(repo_home / 'outputs'/'sacramento'/'sensitivity'/'PED'/'5.17'/os.path.join(f"{str('{:.2f}'.format(round(PED,2)))}_outputs.csv"))
    hh_demand.to_csv(repo_home / 'outputs'/'sacramento'/'sensitivity'/'PED'/'5.17'/os.path.join(f"{str('{:.2f}'.format(round(PED,2)))}_hh_demand.csv"))
    hh_bills.to_csv(repo_home / 'outputs'/'sacramento'/'sensitivity'/'PED'/'5.17'/os.path.join(f"{str('{:.2f}'.format(round(PED,2)))}_hh_bills.csv"))
