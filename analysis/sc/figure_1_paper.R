...
path = "../../outputs/santa_cruz/experiments/review_responses"
...
p_utility_cost <-
  ...
  theme_classic() +xlim(c(ll_date,ul_date))+
...
bills_plot_c <- ggplot() + geom_line(data=bills_c_melted,aes(x=Date,y=value,color=Income_Group))+theme_classic()+ xlim(c(ll_date,ul_date))+
...
bills_plot_m <- ggplot() + geom_line(data=bills_m_melted,aes(x=Date,y=value,color=Income_Group))+theme_classic()+ xlim(c(ll_date,ul_date))+
...
ggsave(p_final,filename = "~/Documents/__college/reseach_stuff/hr2w_droughts/documents/figure_1.pdf",width=5.118,height=3,unit="in",scale=1.7)
